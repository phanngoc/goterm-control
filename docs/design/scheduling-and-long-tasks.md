...
| **P1 — Lịch & heartbeat** | **P1a ✅ lịch**: `schedules`/`schedule_runs` (schema v4, kèm `agents.scratch` cho P1b); vòng quét CAS 30s; `agent`→task `kind=scheduled` (run `pending` tới khi task kết thúc, gateway nào thấy trước thì settle — cũng CAS), `command`→sh trong gateway, output cắt 8KB giữ đầu+cuối; catch-up một lần / `--skip-missed`; bậc thang 30s→1m→5m→15m→1h, báo Telegram lần 2 (cooldown 1h), tự tắt lần 10; kết quả task theo lịch **gửi Telegram** cho chủ (`--quiet` để không); `bomclaw schedule add|list|show|enable|disable|remove|run-now`; RPC `schedules.*`; config `schedules.enabled` mặc định false. **P1b ✅ heartbeat**: hàng `schedules` hệ thống `heartbeat:<agent>` (`payload_kind=heartbeat`, `owner_agent`, `system=1`, `skip_missed=1`) do gateway đồng bộ từ config mỗi lần khởi động (`EnsureHeartbeat`); skip theo thứ tự rẻ→đắt: ngoài `active_hours` → scratch rỗng → agent đang có run (`deps.Runs`) → còn task heartbeat mở → mới tạo task `kind=heartbeat` (`max_continuations=2`); skip không ghi `schedule_runs`, chỉ `last_status=skipped`; `NO_REPLY` → không gửi; khác → `💓 <agent>` Telegram; `bomclaw heartbeat scratch --set/--append/--clear`, `status`, `run-now`; config `heartbeat.{enabled:false, every:"30m", active_hours, tz}`. **P1c ✅ tab Schedules** (PR #104): bảng name/when/next/last/does/owner, drawer chi tiết + runs (output), form tạo (cron/every/at, agent task/shell command, tz, owner, quiet, skip_missed), run-now/enable/disable/remove qua RPC `schedules.*`; poll 10s, drawer 5s | `0 8 * * 1-5` chạy đúng giờ đúng TZ, đúng một lần khi hai gateway (test `TestTwoGatewaysFireOnce`, `TestClaimScheduleIsExclusiveAcrossHandles`); máy tắt qua giờ → bù một lần (`TestMissedScheduleCatchesUpOnce`); heartbeat scratch rỗng → 0 API call |
| ~~**P2 — Cha–con**~~ ✅ | `bomclaw task sub --parent T` (kế thừa `context_id`, `depth+1`, `kind=sub`, cap `MaxOpenChildren=8`, từ chối trên task đã kết thúc); `task block --on children`; **`WakeParents` sweep**: cha `blocked/children` mà mọi con terminal → `submitted`, checkpoint += tóm tắt từng con (state/title/result cắt 1500), `assigned_to = claimed_by` (affinity → `--resume`), event `children-done`, CAS trên `state=blocked` nên hai gateway không đánh thức hai lần; runner gọi ngay sau run của một task con + trong sweep; gateway poke agent được pin. Prompt: mục "Your child tasks" + hướng dẫn `task sub`/depth; `task show` in cây; TaskDrawer mục Children (bấm mở con/cha), card "k/n children finished"; wake `comment`: `msg --task T` → poke, prompt của T có "Messages about this task" rồi `MarkRead` (mail chung để cho chat) | Luồng A (§7) chạy trong test `TestParentIsWokenWithChildResults` (run 2 của cha `--resume` đúng session, prompt có 2 kết quả con); `MaxDepth` từ chối ở depth 6 (`TestCreateSubTaskInheritsAndLimits`) |
| ~~**P3 — Song song trong một agent**~~ ✅ | `tasks.concurrency` (mặc định 1): runner giữ N slot, mỗi claim chạy goroutine riêng, run kết thúc → poke để lấy task kế ngay; chat vẫn lane riêng; `Wait()` join cả run đang chạy | `TestConcurrencyRunsTasksSideBySide`: 3 task, concurrency 2 → đỉnh 2 run cùng lúc; concurrency 1 → đỉnh 1 |
...
- **`payload_kind='heartbeat'`** thay vì `agent` như §5.6 gợi ý: skip rules và `NO_REPLY` là hành vi riêng, tách kind rõ hơn là cắm cờ vào `AgentPayload`. `active_hours` nằm trong payload của hàng (không cần dẫn config vào scheduler); cadence là `spec` của hàng.
- **Đồng bộ từ config**, không phải từ CLI: mỗi khởi động `EnsureHeartbeat` upsert hàng (đổi cadence/giờ → re-arm từ bây giờ, reset streak; giống hệt → không đụng clock); `enabled:false` → tắt hàng nhưng giữ lịch sử. Hàng tự tắt vì 10 lỗi sẽ **bật lại khi restart** — restart là bắt đầu lại. CLI `schedule disable heartbeat:x` vẫn được (tạm dừng), restart kế sẽ bật lại theo config.
- **Skip không ghi `schedule_runs`** (48 hàng "skipped"/ngày sẽ chôn các run thật), chỉ log + `last_status=skipped`. Look thật (tạo task) mới có run `pending` → settle như `agent`.
- Thêm luật skip so với §5.6: **còn task heartbeat mở** (`OpenScheduledTask`) → skip. Che hai trường hợp: `tasks.auto_claim=false` (task không ai nhận, không được chồng 48 task/ngày) và look trước chưa xong.
- Heartbeat cần cả `schedules.enabled` và `tasks.auto_claim` trên cùng gateway; thiếu cái nào gateway log WARNING lúc khởi động. `skip_missed=1` cố định: heartbeat bỏ lỡ lúc máy tắt không phải nợ.
- Prompt heartbeat là *body* của task; taskrunner bọc thêm cơ chế `task done`/ngân sách thời gian như mọi task. Agent trả `NO_REPLY` qua `task done --result` **hoặc** chỉ reply chữ đó — `classify` đều ra `completed` với result đó; `isNoReply` chấp nhận cả backtick/đậm/đuôi giải thích.
- Reporter (#98) bỏ qua task có `schedule_id` nên kết quả heartbeat không bị gửi hai lần.
